...
\label{fig:ctx_new}
\begin{tikzpicture}[
  entity/.style={draw=hitblue,fill=hitblue!10,rectangle,minimum width=2.6cm,minimum height=1.1cm,
                 align=center,font=\small\bfseries,rounded corners=2pt},
  system/.style={draw=hitblue,fill=hitblue!25,ellipse,minimum width=4cm,minimum height=2.5cm,
                 align=center,font=\small\bfseries},
  arr/.style={->,>=Stealth,thick,draw=hitblue!70},
  lbl/.style={font=\scriptsize,fill=white,inner sep=1pt}
]
  \node[system] (sys) at (0,0) {Satellite Crop\\Health Monitor\\System};
  \node[entity] (farmer)   at (-6.5, 2)   {Farmer};
  \node[entity] (agro)     at (-6.5,-2)   {Agronomist};
  \node[entity] (gee)      at  (6.5, 2)   {Google\\Earth Engine};
  \node[entity] (meteo)    at  (6.5,-2)   {Open-Meteo\\Weather API};
  \node[entity] (admin)    at  (0,  4.5)  {System\\Administrator};

  \draw[arr] (farmer) -- node[lbl,above]{Field polygon, login} (sys);
  \draw[arr] (sys)    -- node[lbl,below]{Health status, recommendations} (farmer);
  \draw[arr] (agro)   -- node[lbl,above]{Field selection} (sys);
  \draw[arr] (sys)    -- node[lbl,below]{NDVI trends, heatmap} (agro);
  \draw[arr] (sys)    -- node[lbl,above]{Field polygon, date range} (gee);
  \draw[arr] (gee)    -- node[lbl,below]{NDVI time-series, spatial data} (sys);
  \draw[arr] (sys)    -- node[lbl,above]{Coordinates, date range} (meteo);
  \draw[arr] (meteo)  -- node[lbl,below]{Temperature, rainfall} (sys);
  \draw[arr] (admin)  -- node[lbl,right]{User management, config} (sys);
  \draw[arr] (sys)    -- node[lbl,left]{Audit logs, reports} (admin);
\end{tikzpicture}
\end{figure}
...
\label{fig:activity}
\begin{tikzpicture}[
  act/.style={draw=hitblue,fill=hitblue!10,rounded rectangle,minimum width=5.2cm,
              minimum height=0.75cm,align=center,font=\small},
  dec/.style={draw=hitblue,fill=amber!20,diamond,minimum width=2.2cm,
              minimum height=0.9cm,align=center,font=\scriptsize},
  start/.style={fill=hitblue,circle,minimum size=0.4cm},
  stop/.style={draw=hitblue,circle,minimum size=0.4cm,
               path picture={\fill[hitblue] (0,0) circle (0.15cm);}},
  arr/.style={->,>=Stealth,draw=hitblue!80,thick}
]
  \node[start] (s) at (0,0) {};
  \node[act] (login)  at (0,-1.2) {Farmer Logs In};
  \node[act] (select) at (0,-2.5) {Selects or Creates a Field};
  \node[act] (draw)   at (0,-3.8) {Draws Field Polygon on Map};
  \node[act] (trig)   at (0,-5.1) {Triggers Analysis};
  \node[act] (gee)    at (0,-6.4) {System Queries GEE for Sentinel-2 NDVI};
  \node[dec] (cloud)  at (0,-7.9) {Sufficient\\cloud-free\\scenes?};
  \node[act] (warn)   at (5,-7.9) {Display cloud cover\\warning, use available\\data};
  \node[act] (wx)     at (0,-9.4) {Retrieve Weather Data from Open-Meteo};
  \node[act] (feat)   at (0,-10.7) {Compute Feature Vector};
  \node[act] (ml)     at (0,-12.0) {Apply Logistic Regression Classifier};
  \node[act] (rules)  at (0,-13.3) {Apply Recommendation Rules};
  \node[act] (store)  at (0,-14.6) {Store Analysis in Database};
  \node[act] (disp)   at (0,-15.9) {Display Results, Heatmap, Recommendations};
  \node[stop] (e)     at (0,-17.0) {};
  ...
  \draw[arr] (trig) -- (gee);
  \draw[arr] (gee) -- (cloud);
  \draw[arr] (cloud) -- node[left,font=\scriptsize]{Yes} (wx);
  \draw[arr] (cloud) -- node[above,font=\scriptsize]{No} (warn);
  \draw[arr] (warn) |- (wx);
  \draw[arr] (wx) -- (feat);
  \draw[arr] (feat) -- (ml);
  \draw[arr] (ml) -- (rules);
...
\begin{tikzpicture}[
  class/.style={draw=hitblue,fill=hitblue!5,rectangle split, rectangle split parts=3,
                minimum width=5cm, font=\small, align=left},
  arr/.style={->,>=open triangle 60,draw=hitblue,thick},
  compo/.style={->,>=diamond,draw=hitblue,thick}
]
  % User class
  \node[class] (user) at (0,0) {
    \textbf{User}
    \nodepart{two}
    \texttt{id: String (CUID)} \\
    \texttt{name: String} \\
    \texttt{email: String [unique]} \\
    \texttt{password: String (bcrypt)} \\
  ...
  % Field class
  \node[class] (field) at (7.5,0) {
    \textbf{Field}
    \nodepart{two}
    \texttt{id: String (CUID)} \\
    \texttt{name: String} \\
    \texttt{cropType: String} \\
    \texttt{polygon: String (GeoJSON)} \\
    \texttt{area: Float? (ha)} \\
    \texttt{location: String?} \\
    \texttt{userId: String (FK)} \\
    \texttt{createdAt: DateTime}
    \nodepart{three}
    \texttt{+calculateArea()} \\
    \texttt{+getCentroid()} \\
  ...
  % Analysis class
  \node[class] (analysis) at (7.5,-5.8) {
    \textbf{Analysis}
    \nodepart{two}
    \texttt{id: String (CUID)} \\
    \texttt{fieldId: String (FK)} \\
    \texttt{meanNDVI: Float} \\
    \texttt{ndviTrend: NDVITrend \{enum\}} \\
    \texttt{healthStatus: HealthStatus \{enum\}} \\
    \texttt{avgTemperature: Float} \\
    \texttt{totalRainfall: Float} \\
    \texttt{waterStressRisk: Boolean} \\
    \texttt{diseaseRisk: Boolean} \\
    \texttt{rawData: String (JSON)} \\
    \texttt{recommendations: String} \\
    \texttt{createdAt: DateTime}
    \nodepart{three}
    \texttt{+getRecommendations()} \\
    \texttt{+getHeatmapData()} \\
    \texttt{+getTrendSeries()}
  };

  % Relationships
  \draw[compo] (user.east) -- node[above,font=\scriptsize]{1} node[below,font=\scriptsize]{has many} (field.west);
  \draw[compo] (field.south) -- node[right,font=\scriptsize]{has many} node[left,font=\scriptsize]{1} (analysis.north);
\end{tikzpicture}
\end{figure}
