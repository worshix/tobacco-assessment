...
\begin{tikzpicture}[
  ...
  act/.style={draw=hitblue,fill=hitblue!10,rectangle,rounded corners=5pt,
              ...
              align=center,font=\normalsize},
  % Diamond — aspect ratio > 1 makes it wider relative to height
  dec/.style={draw=hitblue,fill=amber!20,diamond,aspect=2.8,
  ...
  \node[dec]    (cloud)  at (0, -9.0)  {Sufficient\\cloud-free scenes?};
  % "No" branch — to the right, same y as diamond
  \node[act]    (warn)   at (4.5,-9.0) {Display cloud-cover warning;\\proceed with available data};
  \node[act]    (wx)     at (0,-11.0)  {Retrieve Weather Data from Open-Meteo};
  \node[act]    (feat)   at (0,-12.4)  {Compute Feature Vector};
  \node[act]    (ml)     at (0,-13.8)  {Apply Logistic Regression Classifier};
  \node[act]    (rules)  at (0,-15.2)  {Apply Recommendation Rules};
  \node[act]    (store)  at (0,-16.6)  {Store Analysis in Database};
  \node[act]    (disp)   at (0,-18.0)  {Display Results, Heatmap, Recommendations};
  \node[stop]   (e)      at (0,-19.4)  {};
